...
M.setup = function()
	...
	local f = io.open("output.log", "w")
	io.output(f)
	-- io.write(vim.inspect(which_key_nmap))
	-- io.write(vim.inspect(which_key_nopt))
	-- io.write(vim.inspect(which_key_vmap))
	-- io.write(vim.inspect(which_key_vopt))
	-- io.write(vim.inspect(which_key_local_nmap))
	io.write("which_key_vmap\n")
	io.write(vim.inspect(merge_tables(which_key_vmap, which_key_vopt)))
	io.write("which_key_nmap\n")
	io.write(vim.inspect(merge_tables(which_key_nmap, which_key_nopt)))
	io.write("which_key_local_nmap\n")
	io.write(vim.inspect(merge_tables(which_key_local_nmap, which_key_local_nopt)))
	io.close(f)
end
